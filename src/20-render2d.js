...
/* ================= Classic '99 — PAL99 master palette + quantizer ==========
   GFX99-PLAN Phase A (points 2-3). ONE 256-color indexed palette for
   everything Classic mode draws; texture comes from ordered Bayer dither,
   never gradients — dither IS the era's texture. The first 64 indices are 8
   team ramps of 8 shades derived from TEAMS at boot, so team colour is a ramp
   remap (the SC/AoE2 mechanism) and the colourblind palette flows through
   automatically. Render-side only; the sim never sees any of this. */
const R99={};
(function(){
  const P=[];
  const hex=h=>[parseInt(h.slice(1,3),16),parseInt(h.slice(3,5),16),parseInt(h.slice(5,7),16)];
  const L=(a,b,t)=>Math.round(a+(b-a)*t);
  const RS=[],RE=[],LUM=[]; // per-index ramp bounds + luma, for in-ramp dither
  const ramp=(n,c0,c1)=>{const s=P.length;
    for(let i=0;i<n;i++){const t=n===1?0:i/(n-1);
      P.push([L(c0[0],c1[0],t),L(c0[1],c1[1],t),L(c0[2],c1[2],t)]);}
    for(let i=0;i<n;i++){RS[s+i]=s;RE[s+i]=s+n-1;}
    return s;};
  for(let tI=0;tI<8;tI++){ // team ramps: deep shadow -> main -> trim highlight
    const m=hex(TEAMS[tI].main),d=hex(TEAMS[tI].dark),tr=hex(TEAMS[tI].trim);
    ramp(4,[Math.round(d[0]*.45),Math.round(d[1]*.45),Math.round(d[2]*.45)],m);
    ramp(4,m,tr);
  }
  R99.GRASS =ramp(12,[38,52,22],[168,190,110]);   // meadow greens
  R99.OLIVE =ramp(8,[52,58,30],[142,150,88]);     // dry olive grass
  R99.EARTH =ramp(10,[54,38,20],[196,166,112]);   // earth browns
  R99.SAND  =ramp(8,[120,96,58],[228,206,158]);
  R99.STONE =ramp(10,[52,52,58],[200,200,206]);
  R99.TIMBER=ramp(8,[58,38,20],[168,126,74]);
  R99.LEAF  =ramp(10,[14,26,10],[96,128,58]);     // deep foliage / forest floor
  R99.WATER =ramp(8,[18,46,78],[110,178,210]);    // THE cycled ramp
  ...
  R99.DARKS =ramp(6,[10,8,6],[74,66,52]);         // outlines, night, fog hue
  R99.BACK  =ramp(4,[16,22,8],[60,70,40]);        // backdrop greens
  R99.N=P.length;                                 // real colors; the pad below is never matched
  for(let i=0;i<R99.N;i++)LUM[i]=P[i][0]*3+P[i][1]*6+P[i][2];
  while(P.length<256)P.push([255,0,255]);
  R99.P=P;
  let LUT=null;
  R99.lut=()=>{                                   // 32^3 nearest-index cache, built once (~30ms)
    if(LUT)return LUT;
    LUT=new Uint8Array(32768);
    for(let ri=0;ri<32;ri++)for(let gi=0;gi<32;gi++)for(let bi=0;bi<32;bi++){
      const r=ri*8+4,g=gi*8+4,b=bi*8+4;let best=0,bd=1e9;
      for(let i=0;i<R99.N;i++){const c=P[i];
        const dr=r-c[0],dg=g-c[1],db=b-c[2];
        const d=dr*dr*2+dg*dg*4+db*db;            // green-weighted, the eye's own bias
        if(d<bd){bd=d;best=i;}}
      LUT[ri<<10|gi<<5|bi]=best;}
    return LUT;};
  const BAYER=[0,8,2,10,12,4,14,6,3,11,1,9,15,7,13,5];
  /* In-place indexed + Bayer-dithered region of any canvas. Alpha goes 1-bit
     (the hard-pixel contract): <128 is gone, everything else is solid. */
  R99.quant=(c,x0,y0,w,h,amp)=>{
    const g=c.getContext('2d');
    x0=Math.max(0,x0|0);y0=Math.max(0,y0|0);
    w=Math.min(Math.ceil(w),c.width-x0);h=Math.min(Math.ceil(h),c.height-y0);
    if(w<=0||h<=0)return;
    const im=g.getImageData(x0,y0,w,h),d=im.data,LU=R99.lut();
    const A=amp===undefined?18:amp;
    for(let y=0;y<h;y++)for(let x=0;x<w;x++){
      const o=(y*w+x)*4;
      if(d[o+3]<128){d[o+3]=0;continue;}
      const bb=(BAYER[((y+y0)&3)*4+((x+x0)&3)]/15-.5)*A;
      const r=Math.max(0,Math.min(255,d[o]+bb));
      const gg=Math.max(0,Math.min(255,d[o+1]+bb));
      const b2=Math.max(0,Math.min(255,d[o+2]+bb));
      const pc=P[LU[(r>>3)<<10|(gg>>3)<<5|(b2>>3)]];
      d[o]=pc[0];d[o+1]=pc[1];d[o+2]=pc[2];d[o+3]=255;
    }
  ...
  /* Palette-cycled water: four pre-built tile diamonds — CALM dark water
     ...
     walk the ramp per frame. The era's water was mostly flat and dark; the
     life was in a few travelling glints, never in stripes. */
  let WT=null;
  R99.waterTile=f=>{
    if(!WT){
      WT=[];
      const col=i=>{const pc=P[R99.WATER+i];return 'rgb('+pc[0]+','+pc[1]+','+pc[2]+')';};
      for(let fr=0;fr<4;fr++){
        const c=document.createElement('canvas');c.width=IW*2;c.height=IH*2;
        const g=c.getContext('2d');
        g.beginPath();g.moveTo(IW,0);g.lineTo(IW*2,IH);g.lineTo(IW,IH*2);g.lineTo(0,IH);g.closePath();g.clip();
        g.fillStyle=col(2);g.fillRect(0,0,IW*2,IH*2);          // the calm base
        g.fillStyle=col(3);                                     // dithered near-shade
        for(let y=0;y<IH*2;y++)for(let x=(y&1);x<IW*2;x+=2)
          if(((x*7+y*13)>>2)%3===0)g.fillRect(x,y,1,1);
        // a handful of glints whose position walks with the frame
        for(let s=0;s<4;s++){
          const gx=(s*17+fr*6+3)%(IW*2-6),gy=(s*9+fr*3+2)%(IH*2-2);
          g.fillStyle=col(s===0?6:5);
          g.fillRect(gx,gy,s===0?4:3,1);
        }
        WT.push(c);
      }
    }
    return WT[f&3];};
  R99.zoomSnap=z=>[.5,.7,1,1.4,2].reduce((a,b)=>Math.abs(b-z)<Math.abs(a-z)?b:a);
})();
...
function stampWorn(b){
  ...
  tctx.globalAlpha=1;
  if(OPT.r99)R99.quant(terrain,(cx-r)*TREZT,(cy-r)*TREZT,r*2*TREZT,r*2*TREZT); // mid-game stamps stay indexed
}
function stampCleared(x,y){
  ...
  tctx.globalAlpha=1;
  // the stump — species and girth matched to the tree that stood here via
  // getResSpr's own variant hash, NW-lit like everything else on the board
  const hv=hash2(x*13,y*17),pine=hv%3===0;
  const w=(10+(hv%4)*1.7)*.42,ht=w*.95;
  const sx=px+9+(h%8),sy=py+10+((h>>>3)%8);
  const bark=pine?'#4a3018':'#54381e',barkD=pine?'#2e1d0c':'#38240f';
  const rs=mulberry(hv*97+3);
  tctx.fillStyle='rgba(18,26,10,.35)';
  tctx.beginPath();tctx.ellipse(sx+w*.5,sy+1,w*1.6,w*.6,.12,0,7);tctx.fill();
  tctx.fillStyle=bark;
  tctx.beginPath();tctx.ellipse(sx,sy,w*1.25,w*.6,0,0,7);tctx.fill();
  tctx.beginPath();tctx.ellipse(sx-w*.9,sy+w*.1,w*.38,w*.2,0,0,7);tctx.fill();
  tctx.beginPath();tctx.ellipse(sx+w*.85,sy+w*.15,w*.4,w*.2,0,0,7);tctx.fill();
  tctx.fillRect(sx-w,sy-ht,w*2,ht);
  tctx.fillStyle=barkD;tctx.fillRect(sx+w*.15,sy-ht,w*.85,ht);
  tctx.fillStyle='rgba(255,235,200,.16)';tctx.fillRect(sx-w,sy-ht,w*.35,ht);
  tctx.strokeStyle='rgba(20,12,4,.55)';tctx.lineWidth=.7;
  tctx.beginPath();tctx.moveTo(sx-w*.35,sy-ht+.5);tctx.lineTo(sx-w*.3,sy+w*.3);tctx.stroke();
  tctx.beginPath();tctx.moveTo(sx+w*.5,sy-ht+.5);tctx.lineTo(sx+w*.55,sy+w*.2);tctx.stroke();
  tctx.strokeStyle=barkD;tctx.lineWidth=1.2;
  tctx.beginPath();tctx.ellipse(sx,sy-ht,w,w*.55,0,0,7);tctx.stroke();
  tctx.fillStyle='#d8b578';
  tctx.beginPath();tctx.ellipse(sx,sy-ht,w*.9,w*.48,0,0,7);tctx.fill();
  tctx.fillStyle='#eed7a4';
  tctx.beginPath();tctx.ellipse(sx-w*.12,sy-ht-.3,w*.55,w*.28,0,0,7);tctx.fill();
  tctx.strokeStyle='#9a7440';tctx.lineWidth=1;
  tctx.beginPath();tctx.ellipse(sx,sy-ht,w*.45,w*.24,0,0,7);tctx.stroke();
  tctx.fillStyle='#a87e4a';
  tctx.beginPath();tctx.arc(sx,sy-ht,1,0,7);tctx.fill();
  tctx.fillStyle='#c9a266';
  for(let i=0;i<3;i++)tctx.fillRect(sx-w*2.2+rs()*w*1.6,sy+w*.3+rs()*3,1.8,1.2);
  if(OPT.r99)R99.quant(terrain,(px-6)*TREZT,(py-6)*TREZT,(TILE+12)*TREZT,(TILE+12)*TREZT); // felled tiles stay indexed
}
...
function refreshTerrain(){
  ...
  if(G.elev)for(let y=0;y<MAP;y++)for(let x=0;x<MAP;x++){
    ...
    /* Classic '99 (point 12): high ground is a hard TERRACE, not a soft
       contour — the translucent strokes vanish into the Bayer dither after
       the quant. Solid sunlit lip on the NW rim, solid deep drop-shade on
       the SE edge; the jittered wave stays so it reads as landform, not
       lattice. The whole-terrain quant right after makes them indexed. */
    tctx.lineWidth=OPT.r99?3:2.4;
    tctx.strokeStyle=OPT.r99?'#a8be6e':'rgba(240,248,205,.19)'; // = GRASS ramp top: sunlit lip, not a cream path
    tctx.beginPath();
    if(!e(x,y-1))wav(x*TILE,y*TILE+1,x*TILE+TILE,y*TILE+1);
    if(!e(x-1,y))wav(x*TILE+1,y*TILE,x*TILE+1,y*TILE+TILE);
    tctx.stroke();
    tctx.strokeStyle=OPT.r99?'#1c2a12':'rgba(22,34,12,.21)';
    tctx.beginPath();
  ...
  /* Classic '99: the whole painted ground — shores, forest floor, doodads,
     hills — passes through PAL99 + Bayer in one deterministic post-pass.
     Every gradient becomes dither; every colour becomes one of 256. One-time
     cost behind the Start press (~150-400ms), like the rest of genMap. */
  if(OPT.r99)R99.quant(terrain,0,0,terrain.width,terrain.height);
...
function getResSpr(type,h){
  ...
  const sp={c,ox:ax,oy:ay,w:W,h:Ht,sh99:1}; // sh99: Classic bakes a stipple ground shadow
...
function spc(sp){
  ...
  if(sp.sh99)sh99(g,c,sp,eff);  // baked stipple ground shadow (point 11)
  R99.post(c,{amp:10});
  sp[k]=c;return c;
}
/* Point 11's baked shadow for trees / gold crags / berry bushes: their
   painted soft shadows die at the 1-bit alpha cut, so Classic floated them.
   A 50% checkerboard ellipse, sized from the art's own width, composited
   UNDER the art (destination-over) before the quant — same stipple family
   as the unit shadow sheets. */
let PATC=null;
function sh99(g,c,sp,eff){
  const im=g.getImageData(0,0,c.width,c.height).data;
  let x0=c.width,x1=0;
  for(let y=0;y<c.height;y++)for(let x=0;x<c.width;x++)
    if(im[(y*c.width+x)*4+3]>=128){if(x<x0)x0=x;if(x>x1)x1=x;}
  if(x1<=x0)return;
  if(!PATC){PATC=document.createElement('canvas');PATC.width=2;PATC.height=2;
    const pg=PATC.getContext('2d');pg.fillStyle='rgb(24,19,12)';
    pg.fillRect(0,0,1,1);pg.fillRect(1,1,1,1);}
  const rx=(x1-x0)/2*.8,ry=Math.max(3,rx*.38);
  const cx=sp.ox*eff+rx*.28,cy=Math.min(sp.oy*eff,c.height-ry-1);
  g.save();
  g.globalCompositeOperation='destination-over';
  g.fillStyle=g.createPattern(PATC,'repeat');
...
const PAT99={};
function pat99(col,dens){
  const k=col+'/'+dens;
  if(PAT99[k])return PAT99[k];
  const c=document.createElement('canvas');c.width=2;c.height=2;
  const g=c.getContext('2d');g.fillStyle=col;
  g.fillRect(0,0,1,1);if(dens>1)g.fillRect(1,1,1,1);
  PAT99[k]=ctx.createPattern(c,'repeat');return PAT99[k];
}
...
function drawBld(b){
  ...
  if(!bR){
    if(OPT.r99&&!d.thin&&!d.farm&&b.built){
      // era grounding: dirtPad's baked soft shadow dies at the 1-bit alpha
      // cut, so Classic gets the sparse stipple contact ellipse instead
      ctx.fillStyle=pat99('rgb(26,20,12)',1);
      ctx.beginPath();
      ctx.ellipse(pc.x+b.size*3,pc.y+2,b.size*IW*.72,b.size*IH*.66,0,0,7);
      ctx.fill();
    }
...
/* ===== Classic '99 unit sheets (GFX99-PLAN Phase B, points 4-7) =============
   The Blender sheets pass through the era pipeline AT LOAD, not at render
   (the forge's 5-band toon shading is already in them — what was missing was
   the back half of the chain): per-cell downscale to the EXACT on-screen 1x
   size (integer cell walls, dither at output scale), then R99.post — bands,
   PAL99 index + Bayer, 1-bit alpha, warm 1px outline. Team surfaces become a
   TEAM RAMP REMAP via the mask (fetched for player 0 too, so the colourblind
   palette flows through). Cached beside the modern tints under a '/99' key
...
function sprGet99(key,an,p,eff){
  ...
  for(let r=0;r<8;r++)for(let f=0;f<m.frames;f++)   // per-cell keeps cells on integers
    g.drawImage(base,f*cw,r*ch,cw,ch,f*ncw,r*nch,ncw,nch);
  let mi=null;
  if(msk){
    const mc=document.createElement('canvas');mc.width=c.width;mc.height=c.height;
    const mg=mc.getContext('2d');mg.imageSmoothingEnabled=true;
    for(let r=0;r<8;r++)for(let f=0;f<m.frames;f++)
...
/* Point 6: the era shadow — the unit's own silhouette sheared onto the ground
   to the south-east and thresholded to a 50% checkerboard. Mirrored facings
   flip it with the sprite: the authentic baked-sheet quirk. */
function sprShadow99(sheet,cw,ch,ax,ay,frames){
  const pad=Math.ceil(ay*.6)+2,cws=cw+pad;
  const c=document.createElement('canvas');c.width=cws*frames;c.height=ch*8;
  const g=c.getContext('2d');
  for(let r=0;r<8;r++)for(let f=0;f<frames;f++){
    g.save();
    g.beginPath();g.rect(f*cws,r*ch,cws,ch);g.clip();  // no bleed across cells
    // X = dx + .55*(ay-dy)  (lean SE), Y = ay + .3*(ay-dy)  (flatten to ground)
    g.setTransform(1,0,-.55,-.3,f*cws+.55*ay,r*ch+1.3*ay);
    g.drawImage(sheet,f*cw,r*ch,cw,ch,0,0,cw,ch);
    g.restore();
  }
  const im=g.getImageData(0,0,c.width,c.height),d=im.data;
  for(let y=0;y<c.height;y++)for(let x=0;x<c.width;x++){
    const i=(y*c.width+x)*4;
    if(d[i+3]>=100&&((x^y)&1)===0){d[i]=24;d[i+1]=19;d[i+2]=12;d[i+3]=255;}
    else d[i+3]=0;
  }
  g.putImageData(im,0,0);
  return{c,cw:cws,ch};
...
function sprDraw(u,moving){
  ...
  const d=((u._oct%8)+8)%8;
  const f=sprFrame(u,m,an);
  if(OPT.r99){
    /* Phase B: the '99-chain sheet. 8 facings drawn from 5 stored + mirror
  ...
  const cw=meta.cell[0],ch=meta.cell[1],S=SPR_SCALE/(USPR.res||1);
...
function bsprGet99(type,age,team,sl,eff){
  ...
  if(msk){
    ...
    const mg=mc.getContext('2d');mg.imageSmoothingEnabled=true;
    mg.drawImage(msk,0,0,w,h);
    mi=mg.getImageData(0,0,w,h).data;
  }
  if(sl)snowRim(c,sl);
  // amp 6, not the units' 12: half-strength shade dither keeps the big roof
...
function bsprDraw(b,pt0,bl){
  ...
  /* Daniel's call (r99-town-choice, tq-v85): the PROCEDURAL town owns Classic
     at EVERY zoom — through spc() it quantizes at native TREZ res and
     out-details the 158px Blender sources; one design language, no art pop
     between zoom steps. Modern mode keeps the Blender town untouched.
     (bsprGet99 stays below should this ever revert to per-zoom.) */
  if(OPT.r99)return false;
  const d=BLDS[b.type];
...
function drawUnit(u){
  const pp=isoE(u.x,u.y);let px=pp.x,py=pp.y;
  if(px<cullX0||px>cullX1||py<cullY0||py>cullY1)return;
  if(OPT.r99){ // hard-pixel contract: every sprite lands on whole SCREEN pixels
    const z9=G.cam.z;px=Math.round(px*z9)/z9;py=Math.round(py*z9)/z9;
  }
  const moving=u.spd!==undefined?u.spd>.05:!!(u.path&&u.path.length);
  ...
  if(!sprDraw(u,moving)){
    if(OPT.r99){ // rig fallback still needs a ground shadow: the era stipple
      ctx.fillStyle=pat99('rgb(26,20,12)',2);
      ctx.beginPath();ctx.ellipse(big?3:2,4.3,big?11:8,big?4.2:3,.14,0,7);ctx.fill();
    }
...
function drawCorpse(c){
  ...
  // Classic '99: decay is palette-darkening STEPS (the staged sprites) plus a
  // 3-step exit — sprites never fade on a smooth alpha ramp (point 8)
  const a=OPT.r99?(st===2?Math.min(.7,Math.ceil(Math.min(1,c.t/30)*3)/3*.7):1)
                 :(st===2?Math.min(.7,c.t/30*.7):(st?.62:.8));
  ...
  if(!(UNITS[c.type]&&UNITS[c.type].ship)&&c.type!=='ram'&&age>.25){
    ...
    if(OPT.r99){ // checkerboard blood, stepped soak — coverage does the blending
      if(fade>.15){
        const s9=Math.ceil(soak*3)/3;
        ctx.fillStyle=pat99('#5a1410',2);
        ctx.beginPath();ctx.ellipse(cp.x,cp.y+1.5,(5.5+4.5*s9)*(big?1.6:1),(3+2.4*s9)*(big?1.6:1),0,0,7);
        ctx.fill();
      }
    }else{
...
function draw(){
  ...
  ctx.save();
  ctx.scale(z,z);
  // Classic '99 hard-pixel contract: the camera snaps to whole SCREEN pixels
  // (render-side rounding at the transform — G.cam itself is never touched)
  // and nothing is smoothed. Fractional sampling is the #1 modern tell.
  if(OPT.r99)ctx.translate(-Math.round(G.cam.x*z)/z,-Math.round(G.cam.y*z)/z);
  else ctx.translate(-G.cam.x,-G.cam.y);
  ctx.imageSmoothingEnabled=!OPT.r99;
  ...
  /* Classic '99 water: the palette cycle. Four pre-built diamonds whose bands
     walk the water ramp are blitted over visible sea tiles — the whole water
     shimmers by index rotation, the era's own trick. Wall-clock like the 3D
     water (rain keeps falling in pause); min zoom keeps the baked frame. */
  if(OPT.r99&&G.waterList&&!RIG_LOD){
    const wf=(performance.now()/140)|0;
    for(const w of G.waterList){
      const p=isoPt(w.x,w.y);
      if(p.x<vx0-IW||p.x>vx1+IW||p.y<vy0||p.y>vy1)continue;
      if(!tileKnown(w.x,w.y))continue;
      ctx.drawImage(R99.waterTile(wf+((w.x+w.y*3)&3)),p.x-IW,p.y);
    }
  }
  else if(!OPT.r99&&G.waterList)for(const w of G.waterList){
  ...
  /* Classic '99 fog: blit the raw tile-resolution fog canvas with smoothing
     off — inside the iso transform each fog cell lands as a HARD DIAMOND,
     exactly the era's tile fog. Alphas untouched (phone-visibility gotcha);
     the smooth 512px upscale and the drifting mist are the modern look. */
  ctx.drawImage(OPT.r99?fogC:fogS,0,0,MAP,MAP);
  if(!OPT.r99){
  ...
  if(!OPT.r99&&vig&&vig.width&&vig.height)ctx.drawImage(vig,0,0,vw,vh); // no soft vignette in '99
