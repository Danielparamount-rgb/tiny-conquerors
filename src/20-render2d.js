...
(function(){
  ...
  R99.post=(c,opts)=>{
    ...
    const NL=o.bands||6,ST=255/(NL-1);
    ...
    for(let y=0;y<h;y++)for(let x=0;x<w;x++){
      ...
      if(M&&M[i]>=128){
        ...
        let r=Math.round(d[i]/ST)*ST+bb,gg=Math.round(d[i+1]/ST)*ST+bb,b2=Math.round(d[i+2]/ST)*ST+bb;
        r=r<0?0:r>255?255:r;gg=gg<0?0:gg>255?255:gg;b2=b2<0?0:b2>255?255:b2;
        pc=P[LU[(r>>3)<<10|(gg>>3)<<5|(b2>>3)]];
      }
...
function bsprGet99(type,age,team,sl){
  ...
  if(sl)snowRim(c,sl);
  // amp 6, not the units' 12: big soft roof planes sit BETWEEN ramps, and a
  // wide Bayer offset flips them across hues — speckle, not texture
